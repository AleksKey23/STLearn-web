<script>
  ...
  function handleViewResults(testId) {
    if (onViewResults) {
      ...
      <tbody id="tests-table">
        {#each tests as test}
          <tr>
            <td>{test.title}</td>
            <td>
              <span class="status-badge {test.completed ? 'status-completed' : 'status-pending'}">
                {test.completed ? 'Пройден' : 'Не пройден'}
              </span>
            </td>
            <td>{test.completed ? '85%' : '0%'}</td>
            <td>{test.completed ? '100%' : 'Нет данных'}</td>
            <td><button on:click={() => handleViewResults(test.id)}>📊</button></td>
          </tr>
        {/each}
      </tbody>
    </table>
